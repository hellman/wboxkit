...
def ks_expand(key, size=16, expandedKeySize=16*11):
    ...
    res = []
    for i in range(0, expandedKeySize, 16):
...
def SubBytes(state, inverse=False):
    return list(map( (rsbox if inverse else sbox).__getitem__, state))
...
def encrypt_pycrypto(plain, key):
    from Crypto.Cipher import AES
    ct = AES.new(key).encrypt(plain)
    return ct
